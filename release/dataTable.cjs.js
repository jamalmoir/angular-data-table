      ...
      return {
        pre: function pre($scope, $elm, $attrs, ctrl) {
          ...
          if (ctrl.isPinnedRow && ctrl.column.pinnedRowRenderer) {
            var elm = angular.element(ctrl.column.pinnedRowRenderer($elm));
            angular.element(label).append($compile(elm)(cellScope)[0]);
          } else if (ctrl.column.headerTemplate) {
            var _elm = angular.element("<span>" + ctrl.column.headerTemplate.trim() + "</span>");
            angular.element(label).append($compile(_elm)(cellScope));
          } else if (ctrl.column.headerRenderer) {
            var _elm2 = angular.element(ctrl.column.headerRenderer($elm));
            angular.element(label).append($compile(_elm2)(cellScope)[0]);
          } else {
